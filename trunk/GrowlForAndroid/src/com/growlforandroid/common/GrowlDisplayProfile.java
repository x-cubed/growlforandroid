...
public class GrowlDisplayProfile {
	...
	public GrowlDisplayProfile(long id, String name, boolean shouldLog, Integer statusBarDefaults,
			Integer statusBarFlags, Integer toastFlags, URL alert) {
		_id = id;
		_name = name;
	...
	private void displayNotificationInStatusBar(Context context, GrowlNotification notification, Intent defaultIntent) {
		...
		Intent onClick = defaultIntent;
		URL callbackUrl = notification.getCallbackUrl();
		if (callbackUrl != null) {
			onClick = Utility.createLaunchBrowserIntent(callbackUrl);
		}
		statusBarPanel.contentIntent = PendingIntent.getActivity(context, 0, onClick, 0);
	...
	private void displayNotificationAsToast(Context context, GrowlNotification notification) {
		...
		// FIXME: Find a way to enqueue the displaying of the Toast on the UI
		// threads Looper
